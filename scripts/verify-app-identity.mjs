...
const SKIP_DIRS = new Set([
  "node_modules", ".git", "dist", "build", ".output", ".vinxi", ".nitro",
  "capacitor/www", ".lovable", "android/.gradle", "android/build",
]);
...
mustContain("capacitor.config.ts", new RegExp(`appId:\\s*"${APP_ID.replace(/\./g, "\\.")}"`), "appId harus com.mcm.privateconnect");
mustContain("android/app/build.gradle", new RegExp(`applicationId "${APP_ID.replace(/\./g, "\\.")}"`), "applicationId harus com.mcm.privateconnect");
mustContain("android/app/src/main/res/values/strings.xml", new RegExp(APP_ID.replace(/\./g, "\\.")), "package_name harus com.mcm.privateconnect");
...
if (existsSync("android/app/build.gradle")) {
  ...
  if (ids.length !== 1) errors.push(`android/app/build.gradle: ditemukan ${ids.length} applicationId`);
  if (/applicationIdSuffix/.test(gradle)) errors.push("android/app/build.gradle: applicationIdSuffix tidak diizinkan");
  if (/productFlavors/.test(gradle)) errors.push("android/app/build.gradle: productFlavors tidak diizinkan (satu produk saja)");
}
