...
describe("kursor keyset pesan", () => {
  it("kursor memakai pasangan (created_at, id)", () => {
    expect(cursorOf(m("b", "2026-01-01T00:00:00Z"))).toEqual({
  ...
  it("halaman lama tidak melewati pesan dengan timestamp sama", () => {
    const t = "2026-01-01T00:00:00Z";
    const all = [m("a", t), m("b", t), m("c", t), m("d", "2025-12-31T23:59:59Z")].sort(
      compareMessages,
    );
    ...
    const older = all.filter(
      (r) =>
        r.created_at < cursor.createdAt || (r.created_at === cursor.createdAt && r.id < cursor.id),
    );
