...
describe("akses PIN", () => {
  ...
  it("tidak ada grant select penuh pada profiles ke anon/authenticated setelah pencabutan", () => {
    ...
    const fullGrant = lastIndexOfPattern(/grant select on public\.profiles to (anon|authenticated)/);
  ...
  it("PIN hanya dibaca lewat RPC security definer yang dibatasi", () => {
    for (const fn of [/my_pin\(\)/, /pins_for_me\(uuid\[\]\)/, /find_profile_by_pin\(text\)/]) {
      expect(has(new RegExp(`revoke (all|execute) on function public\\.${fn.source} from [^;]*`))).toBe(true);
      expect(has(new RegExp(`grant execute on function public\\.${fn.source} to authenticated`))).toBe(true);
      expect(has(new RegExp(`grant execute on function public\\.${fn.source} to (anon|public)`))).toBe(false);
    }
  });
  ...
  it("customers.pin dicabut dan hanya lewat customer_pin() untuk pengelola bisnis", () => {
    ...
    expect(has(/customer_pin[\s\S]{0,400}?can_manage_business\(c\.business_id, auth\.uid\(\)\)/)).toBe(true);
...
describe("izin bucket avatars", () => {
  ...
  it("tidak ada policy avatars yang terbuka untuk anon/public setelah pengetatan", () => {
    ...
    const open = lastIndexOfPattern(/bucket_id = 'avatars'[^;]{0,200}(to anon|to public|using \(true\))/);
...
describe("hapus anggota percakapan (jalur admin)", () => {
  ...
  it("is_conv_admin hanya mengakui pembuat percakapan atau role admin/owner", () => {
    const fn = sql.slice(lastIndexOfPattern(/create or replace function public\.is_conv_admin\(_conv uuid, _uid uuid\)/));
  ...
  it("policy delete tidak pernah dibuat tanpa pengecekan admin", () => {
    const deletes = sql.match(/create policy "[^"]*" on public\.conversation_members for delete[^;]*/g) ?? [];
...
describe("pengecekan keanggotaan percakapan", () => {
  it("is_conv_member tetap security definer dan tidak bisa dipanggil anon", () => {
    const fn = sql.slice(lastIndexOfPattern(/create or replace function public\.is_conv_member\(_conv uuid, _uid uuid\)/));
    expect(fn.slice(0, 400)).toMatch(/security definer/);
    expect(has(/revoke all on function public\.is_conv_member\(uuid, uuid\) from anon/)).toBe(true);
    expect(has(/grant execute on function public\.is_conv_member\(uuid, uuid\) to anon/)).toBe(false);
  });

  it("policy pesan/anggota selalu bersandar pada keanggotaan, bukan true", () => {
    const policies =
      sql.match(/create policy "[^"]*" on public\.(messages|conversation_members|conversations)[^;]*/g) ?? [];
  ...
  it("helper internal bisnis tidak dapat dieksekusi klien", () => {
    expect(has(/revoke all on function public\.business_role_of\(uuid, uuid\) from public, anon, authenticated/)).toBe(
      true,
    );
