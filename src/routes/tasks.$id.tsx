...
export const Route = createFileRoute("/tasks/$id")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Rincian perintah penyiapan: item, jumlah, foto, dan lokasi." },
...
function PhotoRow({ photo }: { photo: { id: string; storage_path: string; caption: string; location_label: string; maps_url: string } }) {
  ...
  return (
    <div className="flex items-center gap-3 rounded-xl border border-border p-2">
      {url ? (
        <img src={url} alt={photo.caption || "Foto tugas"} className="size-16 shrink-0 rounded-lg object-cover" />
      ) : (
        <div className="size-16 shrink-0 rounded-lg bg-muted" />
      )}
      <div className="min-w-0 flex-1 text-xs">
        {photo.caption && <p className="truncate font-medium">{photo.caption}</p>}
        {photo.location_label && <p className="truncate text-muted-foreground">{photo.location_label}</p>}
        {photo.maps_url && (
...
function TaskDetail() {
  ...
  useEffect(() => {
    const channel = supabase
      ...
      .on("postgres_changes", { event: "*", schema: "public", table: "preparation_jobs", filter: `id=eq.${id}` }, () => {
        void qc.invalidateQueries({ queryKey: ["tasks", "detail", id] });
      })
      .on("postgres_changes", { event: "*", schema: "public", table: "preparation_job_items", filter: `job_id=eq.${id}` }, () => {
        void qc.invalidateQueries({ queryKey: ["tasks", "detail", id] });
      })
      .on("postgres_changes", { event: "*", schema: "public", table: "preparation_item_photos", filter: `job_id=eq.${id}` }, () => {
    ...
    return (
      <AppShell header={<MobileHeader title="Detail Tugas" back variant="gradient" />}>
        <EmptyState
          ...
          icon={MessageCircle}
          description={jobQuery.error instanceof Error ? jobQuery.error.message : "Tugas mungkin sudah dihapus."}
          action={
  ...
  const employeeName = (employeesQuery.data ?? []).find((e) => e.id === job.assigned_user_id)?.name ?? "Pegawai";
  ...
  return (
    <AppShell header={<MobileHeader title={job.code} back variant="gradient" />}>
      <div className="space-y-3 p-3">
        {isManager && <ManagerTaskCard job={job} employeeName={employeeName} onChanged={() => void jobQuery.refetch()} />}
        ...
        {!isManager && (
          <div className="card-soft space-y-2 p-3 text-sm">
            ...
            <p className="text-xs text-muted-foreground">Kedaluwarsa: {tanggalPanjang(job.expires_at)}</p>
            {job.notes && <p className="rounded-lg bg-muted px-2 py-1.5 text-xs">Catatan: {job.notes}</p>}
            {job.conversation_id && (
        ...
        <div className="card-soft space-y-3 p-3">
          ...
          <ul className="space-y-3">
            {job.items.map((item) => (
              <li key={item.id} className="space-y-2 border-t border-border pt-2 first:border-t-0 first:pt-0">
                ...
                <p className="text-xs text-muted-foreground">
                  ...
                  {item.actual_qty_base === null ? "belum diisi" : `${Number(item.actual_qty_base)} (satuan dasar)`}
                </p>
                {item.notes && <p className="rounded-lg bg-muted px-2 py-1 text-xs">{item.notes}</p>}
